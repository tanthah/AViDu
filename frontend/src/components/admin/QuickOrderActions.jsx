...
export function TestOrderGenerator({ onGenerate }) {
  ...
  const generateTestOrder = async () => {
    try {
      ...
      // Thông báo
      alert('⚠️ Để tạo đơn hàng test:\n\n' +
        '1. Đăng nhập với tài khoản user\n' +
        '2. Thêm sản phẩm vào giỏ hàng\n' +
        '3. Tiến hành đặt hàng\n' +
        '4. Đăng nhập lại với tài khoản admin\n' +
        '5. Cập nhật trạng thái đơn hàng lên "Hoàn thành"\n' +
        '6. Đăng nhập lại user để đánh giá sản phẩm');
...
export function BulkOrderUpdate({ selectedOrders, onUpdate }) {
  ...
  const handleBulkUpdate = async (targetStatus) => {
    if (!window.confirm(`Cập nhật ${selectedOrders.length} đơn hàng sang trạng thái ${targetStatus}?`)) {
      return;
    }
